...
// 获取错误消息
export function getErrorMessage(locale, errorType, params) {
    var message = ERROR_MESSAGES[locale][errorType];
    if (params) {
        return __assign(__assign({}, message), { description: message.description.replace(/\{(\w+)\}/g, function (_, key) { return params[key] || "{".concat(key, "}"); }) });
    }
    return message;
}
